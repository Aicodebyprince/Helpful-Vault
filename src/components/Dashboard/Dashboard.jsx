  ...
  return (
    <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-4 sm:p-6 border border-slate-200/50 shadow-sm">
      ...
      <div className="space-y-3">
        {upcomingItems.length === 0 && !showAddForm ? (
...
// --- NEW MODAL COMPONENT FOR STICKY NOTES ---
          ...
          {sortedNotes.length === 0 && !showAddForm ? (
            ...
            sortedNotes.map((note) => (
              <button
                ...
                className={`w-full text-left rounded-xl p-3 border shadow-sm relative group transition-all duration-200 ${note.is_pinned ? 'bg-yellow-50 border-yellow-200' : 'bg-white/50 border-slate-200 hover:bg-white'}`}
              >
                {note.is_pinned && (<div className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 rounded-full flex items-center justify-center shadow-md"><Pin className="w-3 h-3 text-white" /></div>)}
                <div className="flex items-start justify-between mb-2">
                  <h4 className="font-semibold text-slate-900 text-sm truncate pr-16">{note.title}</h4>
                  <div className="absolute top-2 right-2 flex items-center space-x-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    <div onClick={(e) => { e.stopPropagation(); onTogglePin(note.id, note.is_pinned); }} className="p-2 rounded-full hover:bg-white/70" title="Pin note"><Pin className="w-4 h-4 text-slate-500" /></div>
                    <div onClick={(e) => { e.stopPropagation(); onDelete(note.id); }} className="p-2 rounded-full hover:bg-white/70" title="Delete note"><Trash2 className="w-4 h-4 text-slate-500 hover:text-red-600" /></div>
                  </div>
                </div>
                <p className="text-sm text-slate-700 leading-relaxed break-words line-clamp-3">{note.content}</p>
              </button>
            ))
          )}
        </div>
